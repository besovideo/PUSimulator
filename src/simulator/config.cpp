...
void LoadConfig(PUConfig* pConfig)
{
    ...
    pConfig->satelliete = GetPrivateProfileIntA("info", "satellite", 2, CONFIG_FILE_PATH_NAME);
    if (0 >= pConfig->satelliete || pConfig->satelliete > 128)
    ...
    pConfig->serverUDPPort = GetPrivateProfileIntA("server", "udp_port", DEFAULT_SERVERUDPPORT, CONFIG_FILE_PATH_NAME);
    if (0 >= pConfig->serverUDPPort || pConfig->serverUDPPort >= 65535)
        pConfig->serverUDPPort = DEFAULT_SERVERUDPPORT;
    pConfig->serverTCPPort = GetPrivateProfileIntA("server", "tcp_port", DEFAULT_SERVERTCPPORT, CONFIG_FILE_PATH_NAME);
    if (0 >= pConfig->serverTCPPort || pConfig->serverTCPPort >= 65535)
        pConfig->serverTCPPort = DEFAULT_SERVERTCPPORT;
...
int SetConfig(const PUConfig* pConfig)
{
    ...
    sprintf(tempbuf, "%d", pConfig->satelliete);
    WritePrivateProfileStringA("info", "satellite", tempbuf, CONFIG_FILE_PATH_NAME);
    WritePrivateProfileStringA("server", "ip", pConfig->serverIP, CONFIG_FILE_PATH_NAME);
    sprintf(tempbuf, "%d", pConfig->serverUDPPort);
    WritePrivateProfileStringA("server", "udp_port", tempbuf, CONFIG_FILE_PATH_NAME);
    sprintf(tempbuf, "%d", pConfig->serverTCPPort);
    WritePrivateProfileStringA("server", "tcp_port", tempbuf, CONFIG_FILE_PATH_NAME);
