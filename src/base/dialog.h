...
public:
    ...
    virtual BVCU_Result OnOpenRequest(BVCSP_DialogParam* pParam) = 0;             // 收到打开请求，回复是否同意，0：同意
...
public:
    ...
    void SetHialog(BVCSP_HDialog hDialog, int mediaDir)
    {
        m_hDialog = hDialog;
        m_openMediaDir = mediaDir;
    }
...
public:
    ...
    virtual BVCU_PUCFG_PTZAttr* OnGetPTZParam() = 0;                         // 收到平台发来的云台查询命令。
